...
void initWS2812B(ws2812b_t* string, SPI_HandleTypeDef* spiHandle, pindef_t spiCS, uint16_t numPixels)
{
	...
	// 9 bytes per pixel + 1 byte on beginning and end
	string->numBytes = (numPixels<<3) + numPixels + 2;
	string->pixelDataArray = (uint8_t *)malloc(string->numBytes);
	string->pixelDataArray[0] = 0; //Clear the preamble byte
	string->pixelDataArray[string->numBytes - 1] = 0; //Clear the post send cleardown byte.
}
...
{
   uint8_t *bptr = string->pixelDataArray + (n<<3) + n + 1;
...
void clearPixels(ws2812b_t* string)
{
	...
	uint8_t *bptr = string->pixelDataArray + 1;
	uint8_t *tPtr;
