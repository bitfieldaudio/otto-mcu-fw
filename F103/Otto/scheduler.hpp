...
namespace otto::mcu {
  ...
  struct Scheduler {
    /// Move-only function with max size of 16
    using Function = fixed_size_function<void(), 16, construct_type::move>;
    /// Move only task type
    struct [[gnu::packed]] TaskElement {
      constexpr static clock::duration nrepeat = clock::duration(-1);
      /// Function to execute
      Function func;
    ...
    bool exec()
    {
      ...
      if (queue_.front().time <= time) {
        ...
        task.func();
        if (task.repeat != TaskElement::nrepeat) {
    ...
    void schedule(clock::duration delay, clock::duration repeat, Function f)
    {
    ...
    {
      schedule(delay, TaskElement::nrepeat, std::move(f));
    }
    void schedule(Function f)
    {
      schedule(clock::duration::zero(), TaskElement::nrepeat, std::move(f));
    }
  ...
  private:
    ...
    util::local_vector<TaskElement, 256> queue_;
  };
