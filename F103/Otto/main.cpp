...
namespace otto::mcu::instances {
  ...
  std::array<Encoder, 4> encoders = {{
    {GPIO_PIN(ENC_A_1), GPIO_PIN(ENC_B_1)},
    {GPIO_PIN(ENC_A_2), GPIO_PIN(ENC_B_2)},
    {GPIO_PIN(ENC_A_3), GPIO_PIN(ENC_B_3)},
    {GPIO_PIN(ENC_A_4), GPIO_PIN(ENC_B_4)},
  }};
...
namespace otto::mcu::power {
  enum struct State { on, shutdown } state = State::on;
  GpioPin power_switch = GPIO_PIN(PWR_BUTTON);
  GpioPin power_hold = GPIO_PIN(PWR_HOLD, .invert = true);
  ...
  void init()
  {
    ...
    power_hold.init(GpioPin::Mode::output_pp);
